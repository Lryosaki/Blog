@extends('main3')

@section('title',"| $category->name ")

@section('content')

<section id="page-breadcrumb">
        <div class="vertical-center sun">
             <div class="container">
                <div class="row">
                    <div class="action">
                        <div class="col-sm-12">
                            <h1 class="title"></h1>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    
    </section> 
<div class="row">
	<div class="col-md-6 col-md-offset-2 ">
		<h1>{{ $category->name }} Category <small>{{ $category->posts()->count() }} Posts</small></h1>
	</div>
	<div class="col-md-2 	">
	@if(Auth::user()->name == 'Alaa Souiba')
		<a href="{{ route('categories.edit', $category->id)}}" class="btn btn-primary btn-block pull-right btn-h2-spacing hvr-grow">Edit</a>

		{{ Form::open(['route' => ['categories.destroy', $category->id], 'method' => 'DELETE']) }}
				{{ Form::submit('Delete', ['class' => 'btn btn-danger btn-block btn-h1-spacing  hvr-grow']) }}
		{{ Form::close() }}
		@endif
		

	{{ Html::linkRoute('categories.index','<< See all tags',[],['class'=>'btn btn-default btn-block btn-h1-spacing hvr-float-shadow'])}}
	</div>
	
	
					
</div>
<div class="row">
	<div class="col-md-6 col-md-offset-2 ">
		<table class="table table-striped">
			<thead>
				<tr>
					<th>#</th>
					<th>Title</th>
					<th></th>
				</tr>
			</thead>
			<tbody > 
			@foreach($category->posts as $post)
				<tr>
					<th> {{$post->id}}</th>
					<td> {{$post->title}}</td>
					<td> 
						
					</td>
					<td>
					<a href="{{route('posts.show',$post->id)}}" class="btn btn-default btn-sm pull-right hvr-grow">View</a>
					</td>
				</tr>
			@endforeach
			</tbody>
		</table>
	</div>
</div>
@stop